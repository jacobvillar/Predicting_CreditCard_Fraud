...
#v1-V28 are values that resulted from PCA because of confidentiality issues. 
...
#The machine learning classification algorithm will be biased towards always predicting the most prevalent class.
...
corrplot(r,type= "lower", t1.col = "black", t1.srt = 15)
...
set.seed(1337) #get similar results whenever we run
...
dim(train.orig)/dim(creditcardFraud) #70% train

dim(test)/dim(creditcardFraud) #30% test
...
prop.table(table(train.smote$class)) #50% fraud (49.80%)

#Class Distribution of ADASYN Balanced Dataset

prop.table(table(train.adas$class)) #50% fraud 

#Class Distribution of DB SMOTE Balanced Dataset

prop.table(table(train.dbsmote$class)) #48% fraud
...
#cv, number  - cross validation 10 times
...
### Recall is the most important assessment for a fraud data. Why? - Prioritizing True Positive (Identifying Fraud Incidents as fraud). The cost of a fraudulent transaction is much higher than the cost of incorrectly identifying a non-fraudulent transaction as fraudulent
